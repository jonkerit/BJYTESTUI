...
- (void)makeSubviewsAndConstraints {
    self.view.backgroundColor = [UIColor bjl_colorWithHex:0X222733];
    ...
    blackCoverView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
...
- (void)makeObservingForLoadingVM:(nullable BJLLoadingVM *)loadingVM isReload:(BOOL)reload {
    ...
    loadingVM.suspendBlock = ^(BJLLoadingStep step,
        ...
        if (enterWrongTemplate) {
            error = BJLErrorMake(BJLErrorCode_enterRoom_unsupportedClient, @"当前 UI 不支持专业小班课以外的班型");
        }
        self.error = error;
