...
- (void)prepareToOpen {
    ...
    self.relativeRect = [self rectInBounds:CGRectMake((1 - relativeWidth) / 2.0, (1 - relativeHeight) / 4.0, relativeWidth, relativeHeight)];
...
- (void)setupSubviews {
    ...
    [self.titleNameLabel bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        ...
        make.height.equalTo(@(BJLIcAppearance.userWindowDefaultBarHeight));
        make.left.equalTo(self.view).offset(BJLIcAppearance.writingBoradToolbarSmallSpace);
    }];
    ...
    self.currentUserNameContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityLabel = BJLKeypath(self, currentUserNameContainerView);
        view.backgroundColor = BJLIcTheme.separateLineColor;
        view.layer.cornerRadius = BJLIcAppearance.toolboxCornerRadius;
        view.hidden = YES;
        view;
    });
    self.currentUserNameLabel = ({
        UILabel *label = [UILabel new];
        label.backgroundColor = [UIColor clearColor];
        label.textColor = BJLIcTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:12];
        label.textAlignment = NSTextAlignmentCenter;
        label.accessibilityLabel = BJLKeypath(self, currentUserNameLabel);
        label;
    });
    [self.view addSubview:self.currentUserNameContainerView];
    [self.currentUserNameContainerView addSubview:self.currentUserNameLabel];
    [self.currentUserNameContainerView bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.top.equalTo(self.topBar.bjl_bottom).offset(10);
        make.height.equalTo(@(30));
        make.centerX.equalTo(self.view);
        make.left.greaterThanOrEqualTo(self.view).offset(50);
        make.right.lessThanOrEqualTo(self.view).offset(-50);
    }];
    [self.currentUserNameLabel bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.center.equalTo(self.currentUserNameContainerView);
        make.equalTo(self.currentUserNameContainerView).insets(UIEdgeInsetsMake(2, 8, 2, 8));
    }];
...
- (void)setupObservers {
    ...
    [self bjl_kvo:BJLMakeProperty(self, currentLayer)
         observer:^BOOL(NSString * _Nullable value, NSString * _Nullable oldValue, BJLPropertyChange * _Nullable change) {
             bjl_strongify(self);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (![value isEqualToString:oldValue]) {
                [self showUserName];
                [self performSelector:@selector(hideUserName) withObject:nil afterDelay:3];
            }
        });
        return YES;
    }];
...
- (void)hideUserName {
    self.currentUserNameContainerView.hidden = YES;
}

- (void)showUserName {
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideUserName) object:nil];
    if (self.writingBoard.status == BJLIcWriteBoardStatus_teacherGathered
        || self.writingBoard.status == BJLIcWriteBoardStatus_teacherPublished) {
        self.currentUserNameContainerView.hidden = NO;
        self.currentUserNameLabel.text = self.currentShowUser.displayName;
    }
    else {
        self.currentUserNameContainerView.hidden = YES;
...
- (void)updateUserListView {
    if (self.boardUserListView.hidden) {
        return;
    }
...
- (void)setStatus:(BJLIcWriteBoardStatus)status {
    ...
    switch (status) {
        case BJLIcWriteBoardStatus_teacherEditing:
            ...
            enableWritingBoard = YES;
            break;
            
        case BJLIcWriteBoardStatus_teacherShare:
            if (self.room.loginUser.isTeacher) {
                enableWritingBoard = NO;
            }
            break;
    ...
    [self.room.drawingVM updateWritingBoardEnabled:enableWritingBoard];
    ...
    if (status != BJLIcWriteBoardStatus_teacherShare
        && (!(status == BJLIcWriteBoardStatus_teacherPublished && (self.writingBoard.duration > 0)) || !(status == BJLIcWriteBoardStatus_studentEdit && (self.writingBoard.duration > 0)))) {
        self.titleNameLabel.text = @"小黑板";
    }
...
- (void)updateCountDownTip {
    ...
    if (leftTimeInterval <= 0.0 ) {
        ...
        NSString *titleName = @"小黑板";
        NSAttributedString *titleAttributedText = [[NSAttributedString alloc] initWithString:titleName ?: @"小黑板"
                                                                                    attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14.0],
                                                                                                 NSForegroundColorAttributeName : BJLIcTheme.viewTextColor}];
        [string appendAttributedString:titleAttributedText];

        if (self.writingBoard.status == BJLIcWriteBoardStatus_teacherPublished) {
            NSString *time = [NSString stringWithFormat:@"%@后收回", [self stringFromTimeInterval:leftTimeInterval]];
            NSAttributedString *timeAttributedText = [[NSAttributedString alloc] initWithString:time
                                                                                        attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12.0],
                                                                                                     NSForegroundColorAttributeName : BJLIcTheme.viewSubTextColor}];
            [string appendAttributedString:timeAttributedText];
        }
        else if (self.writingBoard.status == BJLIcWriteBoardStatus_studentEdit) {
            NSString *time = [self stringFromTimeInterval:leftTimeInterval];
            NSAttributedString *timeAttributedText = [[NSAttributedString alloc] initWithString:time
                                                                                        attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12.0],
                                                                                                     NSForegroundColorAttributeName : BJLIcTheme.viewSubTextColor}];
            [string appendAttributedString:timeAttributedText];
        }
        self.titleNameLabel.attributedText = string;
    }
}
