//
//  BJLIcUserMediaInfoView+pad1to1.m
...
@implementation BJLIcUserMediaInfoView (pad1to1)
...
- (void)makePad1to1Subviews {
    ...
    self.lossRateLabel = ({
        ...
        label.numberOfLines = 1;
        label.lineBreakMode = NSLineBreakByTruncatingMiddle;
        label.accessibilityLabel = BJLKeypath(self, lossRateLabel);
        [self addSubview:label];
        label;
    });
    ...
    self.likeButton = ({
        ...
        NSString *imageName = @"bjl_ic_like";
        if ([BJLAward allAwards].count > 1){
            imageName = @"bjl_ic_award_mediainfo";
        }
        [button setImage:[UIImage bjlic_imageNamed:imageName] forState:UIControlStateNormal];
        [button setImage:[UIImage bjlic_imageNamed:imageName] forState:UIControlStateDisabled];
        [button setImage:[UIImage bjlic_imageNamed:imageName] forState:UIControlStateNormal | UIControlStateDisabled];
    ...
    [self.speakRequestButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        ...
        make.bottom.lessThanOrEqualTo(self.bjl_bottom);
    ...
    [self.allowSpeakRequestButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        ...
        make.width.equalTo(@(BJLIcAppearance.buttonSize)).priorityHigh();
    ...
    self.refuseSpeakRequestButton = ({
        ...
        [button setTitleColor:BJLIcTheme.warningColor forState:UIControlStateNormal];
    ...
    [self.userNameLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.infoGroupView).offset(5.0);
...
- (void)updatePad1to1InfoGroupViewWithReferenceView:(UIView *)referenceView {
    for (UIView *view in referenceView.subviews) {
        // !!!: 解决断网重连后多个infoGroupView叠加的问题
