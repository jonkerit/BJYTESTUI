...
- (void)prepareToOpen {
    ...
    self.minWindowHeight = 230.0f;
    self.minWindowWidth = 460.0;
    ...
    self.relativeRect = [self rectInBounds:CGRectMake((1 - relativeWidth) / 2.0, (1 - relativeHeight) / 4, relativeWidth, relativeHeight)];
}
...
- (void)makeCommanConstraints {
    ...
    self.resetButton = ({
        ...
        button.backgroundColor = [BJLIcTheme subButtonBackgroundColor];
    ...
    [containerView bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        ...
        make.top.equalTo(self.resultContainerView).offset(BJLIcAppearance.userWindowDefaultBarHeight);
...
- (void)startResponderTimer {
    ...
    [self.endButton setTitle:[NSString stringWithFormat:@"结束(%td)", countDownTime] forState:UIControlStateNormal];
    self.responderTimer = [NSTimer bjl_scheduledTimerWithTimeInterval:1.0 repeats:YES block:^(NSTimer * _Nonnull timer) {
        bjl_strongify(self);
        countDownTime--;
