...
- (void)prepareToOpen {
    self.caption = @"答题器";
    CGFloat optionHeight = [self.answerSheet.options count] < 5 ? BJLIcAppearance.questionAnswerOptionButtonHeight : BJLIcAppearance.questionAnswerOptionButtonHeight * 2 + 25;
    ...
    CGFloat relativeWidth = self.minWindowWidth / (self.view.superview.bounds.size.width ?: 600.0);
    CGFloat relativeH = self.minWindowHeight / (self.view.superview.bounds.size.height ?: 300.0);

    CGFloat relativeHeight = isIphone ? relativeH: [self relativeHeightWithRelativeWidth:relativeWidth width:self.minWindowWidth height:self.minWindowHeight];
    CGFloat relativeX = (1.0 - relativeWidth) / 2.0;
...
- (void)makeSubViews {
    ...
    [self.countDownTipLabel bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.top.equalTo(self.containerView).offset(BJLIcAppearance.userWindowDefaultBarHeight + 8);
    ...
    [self.countDownTimeLabel bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.top.equalTo(self.containerView).offset(BJLIcAppearance.userWindowDefaultBarHeight + 10);
    ...
    if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
        optionsViewHeight = ([self.answerSheet.options count] <= 4
                             ...
                             : BJLIcAppearance.questionAnswerOptionButtonHeight * 2 + (isIphone ? 10 : 25)); // 1~2 行选项
...
- (void)updateCountDownShowTime{
    ...
    self.countDownTimeLabel.text = [NSString stringWithFormat:@"答题倒计时：%02i:%02i", minutes, second];
}
...
- (void)showCorrectAnswer {
    ...
    [tipLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.top.equalTo(self.answerResultContentView).offset(BJLIcAppearance.userWindowDefaultBarHeight + 10);
    ...
    if ([self.selectedOptions count]) {
        ...
        if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
            ...
            ? (BJLIcAppearance.questionAnswerOptionButtonHeight * [self.selectedOptions count] + (isIphone ? 10 : 25) * ([self.selectedOptions count] - 1))
            : (BJLIcAppearance.questionAnswerOptionButtonHeight * 4 + (isIphone ? 10 : 25) * 3);
        }
        else {
            optionsViewHeight = 75;
            optionsViewWidth = BJLIcAppearance.questionAnswerOptionButtonHeight * [self.selectedOptions count] + 25 * ([self.selectedOptions count] - 1);
        }
