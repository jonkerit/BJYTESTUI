...
- (instancetype)initWithRoom:(BJLRoom *)room documentID:(NSString *)documentID relativeX:(CGFloat)relativeX {
    ...
    if (self) {
        ...
        self.documentViewController.shouldSwitchNativePPTBlock = ^(NSString * _Nullable documentID, void (^ _Nonnull callback)(BOOL)) {
            ...
            if (documentID.length && [documentID isEqualToString:self.documentID]) {
                if (self.switchToNativePPTCallback) {
                    self.switchToNativePPTCallback(self.documentViewController, callback);
                }
            }
        };
        self.documentViewController.prevPageIndicatorImage = [UIImage bjlic_imageNamed:@"bjl_ic_ppt_prev"];
        self.documentViewController.nextPageIndicatorImage = [UIImage bjlic_imageNamed:@"bjl_ic_ppt_next"];
        ...
        self.documentViewController.disableChangeImageFrame = YES;
        [self prepareToOpen];
    }
    return self;
}
...
- (void)updateTopBar {
    ...
    self.topBar.captionLabel.textColor = [UIColor whiteColor];
    self.topBar.backgroundColor = [UIColor bjl_colorWithHex:0X313847 alpha:0.3];
    self.bottomBar.backgroundView.hidden = YES;
    self.bottomBar.backgroundColor = [UIColor bjl_colorWithHex:0X313847 alpha:0.3];
    
    [self.topBar.closeButton bjl_setImage:[UIImage bjlic_imageNamed:@"window_close_ppt"] forState:UIControlStateNormal optionalStates:UIControlStateHighlighted];
    [self.topBar.fullscreenButton bjl_setImage:[UIImage bjlic_imageNamed:@"window_fullscreen_ppt"] forState:UIControlStateNormal optionalStates:UIControlStateHighlighted];
    [self.topBar.fullscreenButton bjl_setImage:[UIImage bjlic_imageNamed:@"window_fullscreen_restore_ppt"] forState:UIControlStateSelected optionalStates:UIControlStateHighlighted];
    [self.topBar.maximizeButton bjl_setImage:[UIImage bjlic_imageNamed:@"window_maximize_ppt"] forState:UIControlStateNormal optionalStates:UIControlStateHighlighted];
    [self.topBar.maximizeButton bjl_setImage:[UIImage bjlic_imageNamed:@"window_maximize_restore_ppt"] forState:UIControlStateSelected optionalStates:UIControlStateHighlighted];
}
...
- (void)setupSubviews {
    ...
    self->_prevPageButton = ({
        ...
        [button setImage:[UIImage bjlic_imageNamed:@"window_prevpage_ppt"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjlic_imageNamed:@"window_prevpage_ppt"] forState:UIControlStateNormal | UIControlStateHighlighted];
    ...
    [self.prevPageButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.right.equalTo(self.pageNumberLabel.bjl_left).offset(- BJLIcAppearance.userWindowDefaultBarHeight / 2);
    ...
    self->_nextPageButton = ({
        UIButton *button = [UIButton new];
        [button setImage:[UIImage bjlic_imageNamed:@"window_nextpage_ppt"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjlic_imageNamed:@"window_nextpage_ppt"] forState:UIControlStateNormal | UIControlStateHighlighted];
    ...
    [self.nextPageButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.left.equalTo(self.pageNumberLabel.bjl_right).offset(BJLIcAppearance.userWindowDefaultBarHeight / 2);
        make.top.bottom.equalTo(self.bottomBar);
    ...
    self->_pptRemarkInfoButton = ({
        ...
        [button setImage:[UIImage bjlic_imageNamed:@"window_pptremark_off"] forState:UIControlStateNormal | UIControlStateHighlighted];
        [button setImage:[UIImage bjlic_imageNamed:@"window_pptremark_on"] forState:UIControlStateSelected];
        [button setImage:[UIImage bjlic_imageNamed:@"window_pptremark_on"] forState:UIControlStateSelected | UIControlStateHighlighted];
...
- (void)setupObservers {
    ...
    [self bjl_kvo:BJLMakeProperty(self.room, loginUser)
        ...
        self.resizeHandleImageViewHidden = !isTeacherOrAssistant || (self.state != BJLWindowState_windowed);
    ...
    [self bjl_kvoMerge:@[BJLMakeProperty(self.documentViewController, pageIndex),
                         BJLMakeProperty(self, pageCount)]
              observer:^(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
        bjl_strongify(self);
        self.pageIndex = self.documentViewController.pageIndex;
    ...
    [self bjl_kvo:BJLMakeProperty(self, state)
        ...
        bjl_strongify(self);
        self.documentViewController.disableChangeImageFrame = (self.state == BJLWindowState_windowed);
        if (self.windowInterfaceEnabled) {
            self.resizeHandleImageViewHidden = (self.state != BJLWindowState_windowed);
        }
        BOOL iPad = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad);
        if (iPad) {
...
- (CGRect)windowRectWithDocument:(BJLDocument *)document {
    ...
    return [self rectInBounds:CGRectMake(self.relativeX, 0.0, relativeWidth, relativeHeight)];
}
