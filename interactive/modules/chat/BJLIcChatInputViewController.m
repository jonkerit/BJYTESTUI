...
- (void)makeSubviewsAndConstraints {
    ...
    [self.emoticonButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.left.equalTo(self.containerView.bjl_safeAreaLayoutGuide ?: self.containerView).offset(14.0);
    ...
    self.textView = ({
        ...
        textView.textContainerInset = UIEdgeInsetsMake(BJLIcAppearance.chatViewSmallSpace, BJLIcAppearance.chatViewLargeSpace + margin, BJLIcAppearance.chatViewSmallSpace, BJLIcAppearance.chatViewLargeSpace + margin);
        textView.textColor = [UIColor bjl_colorWithHex:0X333333];
...
- (void)updateEmoticonViewHidden {
    ...
    if (self.emoticonViewController) {
        if (self.emoticonButton.selected) {
            ...
            self.emoticonViewController.popoverPresentationController.backgroundColor = BJLIcTheme.toolboxBackgroundColor;
