...
@interface BJLIcChatViewController () <UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIPopoverPresentationControllerDelegate, BJLPhotoPickerDelegate>
...
- (void)makeSubviewsAndConstraints {
    self.view.backgroundColor = BJLIcTheme.toolboxBackgroundColor;
    // shadow
    ...
    [self.titleLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.view).offset(BJLIcAppearance.chatViewLargeSpace);
    ...
    self.unreadMessagesTipButton = ({
        ...
        button.hidden = YES;
        button.backgroundColor = [UIColor bjl_colorWithHex:0X9FA8B5 alpha:0.5];
        button.titleLabel.font = [UIFont systemFontOfSize:12.0];
        button.titleLabel.textAlignment = NSTextAlignmentCenter;
        button.contentEdgeInsets = UIEdgeInsetsMake(0, BJLIcAppearance.chatViewSmallSpace, 0, BJLIcAppearance.chatViewSmallSpace);
    ...
    [self.unreadMessagesTipButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.right.equalTo(self.view).offset(-BJLIcAppearance.chatViewLargeSpace);
    ...
    self.forbidChatButton = ({
        ...
        button.titleEdgeInsets = UIEdgeInsetsMake(0, BJLIcAppearance.chatViewSmallSpace, 0, 0);
        button.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, BJLIcAppearance.chatViewSmallSpace);
    ...
    [self.forbidChatButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.view).offset(BJLIcAppearance.chatViewLargeSpace);
        make.top.equalTo(self.unreadMessagesTipButton.bjl_bottom).offset(BJLIcAppearance.chatViewLargeSpace);
    ...
    [chatInputView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        if (self.room.loginUser.isStudent) {
            make.top.equalTo(self.tableView.bjl_bottom).offset(BJLIcAppearance.chatViewLargeSpace);
        }
        else {
            make.top.equalTo(self.forbidChatButton.bjl_bottom).offset(BJLIcAppearance.chatViewLargeSpace);
        }
        make.left.equalTo(self.view).offset(BJLIcAppearance.chatViewLargeSpace);
        make.bottom.right.equalTo(self.view).offset(-BJLIcAppearance.chatViewLargeSpace);
        make.height.greaterThanOrEqualTo(@16.0);
    ...
    [self.chatInputEmoticonButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.centerY.equalTo(chatInputView);
        make.left.equalTo(chatInputView).offset(4.0);
        BOOL iPhone = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);
        make.height.width.equalTo(iPhone ? @(24.0) : @(32.0));
    }];
    ...
    [self.chatInputImageButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.centerY.right.equalTo(chatInputView);
        make.width.height.equalTo(self.chatInputEmoticonButton);
    }];
...
- (void)sendText:(NSString *)text {
    if (text.length) {
        ...
        if ([[text substringFromIndex:text.length - 1] isEqualToString:@"\n"]) {
            ...
            [self.chatInputTextButton setTitleColor:BJLIcTheme.viewTextColor forState:UIControlStateNormal];
...
- (void)clearChatInputView {
    ...
    [self.chatInputImageButton setImage:[UIImage bjlic_imageNamed:@"bjl_chat_image_normal"] forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (message) {
        ...
        cell.reloadCellCallback = ^(BJLIcChatTableViewCell * _Nullable cell, NSInteger line) {
            bjl_strongify(self);
            NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
            if (indexPath) {
...
- (void)photoPicker:(BJLPhotoPicker *)BJLPhotoPicker didFinishLoadImageData:(NSArray<ICLImageFile *> *)data failureItems:(NSArray<NSError *> *)failureItems originResult:(BJLPhotoPickerResult *)result {
    ...
    [BJLPhotoPicker.viewController  dismissViewControllerAnimated:YES completion:^{
        bjl_strongify(self);
        [self sendImageMessage:data.firstObject];
    }];
}
...
- (BOOL)atTheBottomOfTableView {
    ...
    return bottomOffset >= 0.0 - BJLIcAppearance.chatCellMinUserInOutTextHeight;
}
