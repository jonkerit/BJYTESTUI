...
- (void)makeSubviewsAndConstraints {
    ...
    self.tableView.rowHeight = BJLIcAppearance.promptCellHeiht;
    ...
    [self.tableView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.top.equalTo(self.view).offset(BJLIcAppearance.promptCellLargeSpace);
...
- (void)enqueueWithPrompt:(NSString *)prompt {
    [self enqueueWithPrompt:prompt duration:BJLIcAppearance.promptDuration];
}
...
- (void)enqueueWithPrompt:(NSString *)prompt duration:(NSInteger)duration important:(BOOL)important {
    while (self.prompts.count >= BJLIcAppearance.promptCellMaxCount) {
        [self.prompts bjl_removeObjectAtIndex:BJLIcAppearance.promptCellMaxCount - 1];
    }
