//
//  BJLIcWindowViewController+protected.m
//  BJLiveUI
//
...
@implementation BJLIcWindowViewController (protected)
...
- (CGFloat)relativeWidthWithRelativeHeight:(CGFloat)relativeHeight aspectRatio:(CGFloat)aspectRatio {
    CGFloat relativeWidth = relativeHeight / BJLIcAppearance.blackboardAspectRatio;
...
- (CGFloat)relativeHeightWithRelativeWidth:(CGFloat)relativeWidth aspectRatio:(CGFloat)aspectRatio {
    CGFloat relativeHeight = relativeWidth * BJLIcAppearance.blackboardAspectRatio;
...
- (void)addHandlers {
    ...
    for (UIView *view in @[self.topBar, self.forgroundView]) {
        ...
        UITapGestureRecognizer *doubleTap = [UITapGestureRecognizer bjl_gestureWithHandler:^(__kindof UIGestureRecognizer * _Nullable gesture) {
            ...
            else if (self.state == BJLWindowState_maximized) {
                [self restore];
            }
        }];
...
- (void)updatePositionAndSize {
    ...
    [self.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        ...
        else if (self.state != BJLWindowState_windowed) {
            return;
        }
        ...
        if (CGPointEqualToPoint(origin, BJLPointNull)) {
            origin = CGPointZero;
        }
...
//        CGFloat minHeightRatio = self.view.superview.bounds.size.height ? (self.minWindowHeight/self.view.superview.bounds.size.height) : size.height;
        ...
        make.width.equalTo(self.view.superview).multipliedBy(MAX(size.width, FLT_MIN));
        make.height.equalTo(self.view.superview).multipliedBy(MAX(size.height, FLT_MIN));
    }];
}
