...
- (void)makeConstraints {
    ...
    self.teacherVideoPlaceholderView = ({
        BJLScVideoPlaceholderView *view = [[BJLScVideoPlaceholderView alloc] initWithImage:[UIImage bjlsc_imageNamed:@"bjl_sc_noteacher"] tip:@"老师不在教室"];
        view.accessibilityLabel = BJLKeypath(self, teacherVideoPlaceholderView);
        view.hidden = YES;
        bjl_return view;
    });
    self.secondMinorVideoPlaceholderView = ({
        BJLScVideoPlaceholderView *view =[[BJLScVideoPlaceholderView alloc] initWithImage:[UIImage bjlsc_imageNamed:@"bjl_sc_noteacher"] tip:@"当前没有学生发言"];
...
- (void)makeViewControllers {
    ...
    self.room.slideshowViewController.shouldSwitchNativePPTBlock = ^(NSString * _Nullable documentID, void (^ _Nonnull callback)(BOOL)) {
        ...
        [alertViewController bjl_addActionWithTitle:@"重新加载"
                                              style:UIAlertActionStyleCancel
                                            handler:^(UIAlertAction * _Nonnull action) {
            callback(NO);
        }];
        [alertViewController bjl_addActionWithTitle:@"跳过动画"
    ...
    if (self.is1V1Class) {
        ...
        if (iPad) {
            ...
            [self.chatButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
                make.left.right.bottom.equalTo(self.segmentView).priorityHigh();
...
//    BOOL iPhone = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);
    ...
    self.fullscreenButton = ({
        ...
        [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_view_full"] forState:UIControlStateNormal];
        button;
    });
    [self.toolView addSubview:self.fullscreenButton];
    [self.fullscreenButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.bottom.equalTo(self.toolView).offset(-BJLScViewSpaceM);
        make.left.equalTo(self.toolView).offset(BJLScViewSpaceS);
        make.width.height.equalTo(@(BJLScControlSize));
    }];
    
    self.eyeProtectedButton = ({
        UIButton *button = [UIButton new];
        button.accessibilityLabel = BJLKeypath(self, eyeProtectedButton);
        [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_eyeProtected_close"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_eyeProtected_open"] forState:UIControlStateSelected];
        [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_eyeProtected_open"] forState:UIControlStateSelected | UIControlStateHighlighted];
        bjl_return button;
    });
    ...
    if (!self.is1V1Class) {
        ...
        self.speakRequestRedDot = ({
            UILabel *view = [UILabel new];
            view.accessibilityLabel = BJLKeypath(self, speakRequestRedDot);
            view.hidden = YES;
        ...
        [self.toolView addSubview:self.speakRequestRedDot];
        [self.speakRequestRedDot bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        ...
        self.noticeButton = ({
            ...
            [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_notice"] forState:UIControlStateNormal];
            bjl_return button;
        });
        [self.toolView addSubview:self.noticeButton];
        [self.noticeButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
            make.left.equalTo(self.fullscreenButton.bjl_right);
            make.top.bottom.width.equalTo(self.fullscreenButton);
        }];
        ...
        if (enableQuestion) {
            self.questionButton = ({
                ...
                [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_question"] forState:UIControlStateNormal];
                bjl_return button;
            });
            self.questionRedDot = ({
                UILabel *view = [UILabel new];
            ...
            [self.questionButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
                make.left.equalTo(self.noticeButton.bjl_right);
        ...
        [self.toolView addSubview:self.eyeProtectedButton];
        [self.eyeProtectedButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
            make.left.equalTo(self.questionButton ? self.questionButton.bjl_right : self.noticeButton.bjl_right);
            make.top.bottom.width.equalTo(self.fullscreenButton);
        }];
    }
    else {
    ...
    self.toolViewController = ({
        ...
        toolViewController.view.accessibilityLabel = BJLKeypath(self, toolViewController);
        toolViewController;
    });
    ...
    // 学生身份才创建 lotteryViewController
    if (self.room.loginUser.isStudent) {
        self.lotteryViewController = [[BJLScLotteryViewController alloc] initWithRoom:self.room];
...
- (void)updateTeacherVideoPlaceholderView {
    ...
    [self.teacherVideoPlaceholderView updateImage:[UIImage bjlsc_imageNamed:@"bjl_sc_noteacher"]];
...
- (void)updateSecondMinorVideoPlaceholderView {
    ...
    [self.secondMinorVideoPlaceholderView updateImage:[UIImage bjlsc_imageNamed:@"bjl_sc_noteacher"]];
