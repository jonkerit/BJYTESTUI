...
- (void)viewDidLoad {
    ...
    self.lotteryLayer = ({
        BJLHitTestView *view = [BJLHitTestView new];
        view.accessibilityLabel = BJLKeypath(self, lotteryLayer);
        bjl_return view;
    });
    [self.view insertSubview:self.lotteryLayer belowSubview:self.eyeProtectedLayer];
...
- (UIButton *)restoreButton {
    if (!_restoreButton) {
        _restoreButton = ({
            ...
            [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_view_restore"] forState:UIControlStateNormal];
            bjl_weakify(self);
...
- (UIButton *)fullScreenEyeProtectedButton {
    if (!_fullScreenEyeProtectedButton) {
        _fullScreenEyeProtectedButton = ({
            UIButton *button = [UIButton new];
            button.accessibilityLabel = BJLKeypath(self, fullScreenEyeProtectedButton);
    ...
    return _fullScreenEyeProtectedButton;
}

- (UIButton *)fullHandUpButton {
    if (!_fullHandUpButton) {
        _fullHandUpButton = ({
            UIButton *button = [UIButton new];
            button.accessibilityLabel = BJLKeypath(self, fullHandUpButton);
            button.backgroundColor = [UIColor clearColor];
    ...
    return _fullHandUpButton;
}

- (UILabel *)fullSpeakRequestRedDot {
    if (!_fullSpeakRequestRedDot) {
        _fullSpeakRequestRedDot = ({
            UILabel *view = [UILabel new];
            view.accessibilityLabel = BJLKeypath(self, fullSpeakRequestRedDot);
            view.hidden = YES;
    ...
    return _fullSpeakRequestRedDot;
}

- (BJLAnnularProgressView *)fullHandProgressView {
    if (!_fullHandProgressView) {
        _fullHandProgressView = ({
            ...
            [self.fullHandUpButton addSubview:progressView];
            [progressView bjl_makeConstraints:^(BJLConstraintMaker *make) {
                make.edges.equalTo(self.fullHandUpButton);
            }];
            progressView;
        });
    }
    return _fullHandProgressView;
}
