...
@interface BJLScMessageOperatorView ()
...
@implementation BJLScMessageOperatorView
...
- (void)updateButtonConstraints {
    ...
    self.stickyMessageButton = [self createButtonWithTitle:self.isSticky ? @"取消置顶" : @"置顶"];
...
- (void)stickyMessage:(UIButton *)button {
    if (self.stickyMessageCallback) {
        self.stickyMessageCallback(!self.isSticky);
    }
}
    ...
    for (UIButton *button in buttonArray) {
        ...
        [button bjl_makeConstraints:^(BJLConstraintMaker *make) {
            ...
            if (lastButton) {
                ...
                make.height.equalTo(@(BJLScMessageOperatorButtonSize)).priorityHigh();
