...
- (instancetype)initForIdiomPad:(BOOL)iPad {
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        self.collectionView = ({
            ...
            [collectionView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        ...
        self.pageControl = ({
            ...
            pageControl.pageIndicatorTintColor = [UIColor bjlsc_grayBorderColor];
            pageControl.currentPageIndicatorTintColor = [UIColor bjlsc_blueBrandColor];
            [self addSubview:pageControl];
            [pageControl bjl_makeConstraints:^(BJLConstraintMaker *make) {
                make.centerX.equalTo(self.bjl_safeAreaLayoutGuide ?: self);
                make.bottom.equalTo(self.bjl_safeAreaLayoutGuide ?: self).with.offset(- BJLScViewSpaceS);
                make.height.equalTo(@8.0);
            }];
            bjl_weakify(self);
            [pageControl bjl_addHandler:^(__kindof UIControl * _Nullable sender, UIControlEvents event) {
                bjl_strongify(self);
...
- (nullable BJLEmoticon *)emoticonAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [self.emoticons bjl_objectAtIndex:index];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self respondsToSelector:@selector(enableInputClicksWhenVisible)]
        && self.enableInputClicksWhenVisible) {
