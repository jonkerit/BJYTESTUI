...
@property (nonatomic) UIButton *unreadMessagesTipButton, *cancelChatButton, *chatInputButton;
...
- (void)setUpSubviews {
    ...
    if (!self.room.featureConfig.enableWhisper) {
        ...
        [whisperButton bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
            make.left.equalTo(self.chatInputButton.bjl_right).offset(2.0);
            make.top.bottom.equalTo(self.chatInputView).inset(4.0).priorityHigh();
            make.right.equalTo(self.chatInputView).offset(-8.0);
    ...
    self.maxStickyTipLabel = ({
        UILabel *label = [UILabel new];
        label.hidden = YES;
        label.text = @"最多可置顶3条信息";
        label.textColor = [UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:12.0];
        label.textAlignment = NSTextAlignmentCenter;
        label.backgroundColor = [UIColor bjl_colorWithHexString:@"#FF1F49" alpha:0.8];
...
- (void)showOperatorViewWithMessage:(BJLMessage *)message
    ...
    BJLScMessageOperatorView *optionView = [[BJLScMessageOperatorView alloc] initWithNeedTranslate:needTranslate needShowOnlyTeacherOrAssistant:needShowOnlyTeacherOrAssistant recallType:recallType canStickyMessage:canStickyMessage isSticky:isSticky];
