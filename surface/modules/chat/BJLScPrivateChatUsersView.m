...
@interface BJLScPrivateChatUsersView ()
...
@implementation BJLScPrivateChatUsersView
...
- (instancetype)initWithRoom:(BJLRoom *)room {
    if (self) {
        ...
        self.backgroundColor = [UIColor bjlsc_lightGrayBackgroundColor];
        [self setUpSubView];
        [self setUpObservers];
    }
    return self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *cellIdentifier = [BJLScPrivateChatUserCell cellIdentifierForUserState:BJLUserState_online
                                                  isTeacherOrAssistant:self.room.loginUser.isTeacherOrAssistant
                                                           isPresenter:NO // need not
                                                              userRole:user.role
                                                              hasVideo:NO
                                                          videoPlaying:NO];
    BJLScPrivateChatUserCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier forIndexPath:indexPath];
...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = ({
            ...
            for (NSString *cellIdentifier in [BJLScPrivateChatUserCell allCellIdentifiers]) {
                [tableView registerClass:[BJLScPrivateChatUserCell class] forCellReuseIdentifier:cellIdentifier];
            }
            tableView;
        });
    }
...
- (UIButton *)emptyListView {
    if (!_emptyListView) {
        _emptyListView = ({
            ...
            [button setTitleColor:[UIColor bjlsc_lightGrayTextColor] forState:UIControlStateHighlighted];
