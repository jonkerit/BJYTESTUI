...
#import "BJLScPrivateChatUsersView.h"
...
@property (nonatomic) BJLScPrivateChatUsersView *privateChatUsersView;
...
@property (nonatomic) BJLCommandLotteryBegin *commandLottery;
...
- (void)makeSubviews {
    ...
    self.textView = ({
        ...
        textView.textContainerInset = UIEdgeInsetsMake(8.0, BJLScViewSpaceM, 8.0, BJLScViewSpaceM);
...
- (void)makeConstraints {
    ...
    [self.privateChatButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.emoticonButton.bjl_right).with.offset(BJLScViewSpaceS);
...
- (void)updateChatStatus:(BJLChatStatus)chatStatus withTargetUser:(nullable BJLUser *)targetUser {
    ...
    // update content
    self.privateChatLabel.textColor = privateChat? [UIColor bjlsc_blueBrandColor] : [UIColor bjlsc_grayTextColor];
    self.privateChatLabel.layer.borderColor = (privateChat? [UIColor bjlsc_blueBrandColor] : [UIColor bjlsc_grayTextColor]).CGColor;
...
- (void)updateCommandLottery:(nullable BJLCommandLotteryBegin *)commandLottery {
...
- (void)textViewDidChange:(UITextView *)textView {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (ABS(height - currentHeight) >= 0.5
            ...
            [UIView animateWithDuration:BJLScAnimateDurationS
                             animations:^{
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([text isEqualToString:@"\n"]) {
        ...
        if (sent) {
            // 发送口令信息 成功
            if (self.commandLottery
                && [self.textView.text isEqualToString:self.commandLottery.command]) {
                [self.room.roomVM requestHitCommandLottery];
            }
            self.textView.text = nil;
            [self textViewDidChange:self.textView];
        }
...
- (void)photoPicker:(BJLPhotoPicker *)BJLPhotoPicker didFinishLoadImageData:(NSArray<ICLImageFile *> *)data failureItems:(NSArray<NSError *> *)failureItems originResult:(BJLPhotoPickerResult *)result {
    ...
    [BJLPhotoPicker.viewController dismissViewControllerAnimated:YES completion:^{
        bjl_strongify(self)
        if (self.selectImageFileCallback) self.selectImageFileCallback(data.firstObject, nil);
    }];
}
...
- (BJLScPrivateChatUsersView *)privateChatUsersView {
    if (!_privateChatUsersView) {
        _privateChatUsersView = [[BJLScPrivateChatUsersView alloc] initWithRoom:self.room];
    }
