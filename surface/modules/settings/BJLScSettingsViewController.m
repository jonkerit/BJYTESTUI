...
@property (nonatomic) UIView *micLabel, *cameraLabel, *beautifyLabel, *backgroundAudioLabel, *pptRemarkLabel, *forbidChatLabel, *forbidSpeakLabel, *forbidAudioLabel, *mirrorModeLabel;
@property (nonatomic) UISwitch *beautifySwitch, *backgroundAudioSwitch, *pptRemarkSwitch, *forbidChatSwitch, *forbidSpeakSwitch, *forbidAudioSwitch, *mirrorModeSwitch;
@property (nonatomic) NSArray<UISwitch *> *allSwitchs;

@property (nonatomic) UIView *separatorLine;

@property (nonatomic) UILabel *contentModeLabel, *videoDefinitionLabel, *cameraPositionLabel, *linkTypeLabel, *pptAnimationLabel;
@property (nonatomic) UIButton *contentModeFitButton, *contentModeFillButton;
@property (nonatomic) UIButton *pptAnimationButton, *pptDisableAnimationButton;
...
@property (nonatomic) NSArray<UIButton *> *videoDefinitionButtons;
...
- (void)makeSubviewsAndConstraints {
    ...
    NSMutableArray *allSwitchs = [NSMutableArray new];
    ...
    [allSwitchs addObject:self.beautifySwitch];
    ...
    [allSwitchs addObject:self.backgroundAudioSwitch];
    ...
    if (self.room.loginUser.isTeacherOrAssistant
        ...
        [allSwitchs addObject:self.forbidChatSwitch];
        switch (self.room.roomInfo.roomType) {
            case BJLRoomType_1vNClass: {
                ...
                [allSwitchs addObject:self.forbidSpeakSwitch];
                // 只有webrtc的班型才有镜像
                if(self.room.featureConfig.isWebRTC) {
                    BJLTuple *mirrorModelControls = [self makeSwitchWithLabel:@"镜像翻转"];
                    BJLTupleUnpack(mirrorModelControls) = ^(UIView *label, UISwitch *swiitch) {
                        self.mirrorModeLabel = label;
                        self.mirrorModeSwitch = swiitch;
                        self.mirrorModeSwitch.on = self.room.recordingVM.isOnEncoderMirrorMode;
                    };
                    [allSwitchs addObject:self.mirrorModeSwitch];
                    tuples = @[micControls, cameraControls, mirrorModelControls, beautifyControls, forbidChatControls, forbidSpeakControls, pptRemarkControls, backgroundAudioControls];
                }
                else {
                    tuples = @[micControls, cameraControls, beautifyControls, forbidChatControls, forbidSpeakControls, pptRemarkControls, backgroundAudioControls];
                }
                break;
            }
...
#pragma clang diagnostic pop
            ...
            case BJLRoomType_doubleTeachersClass: {
                ...
                [allSwitchs addObject:self.forbidAudioSwitch];
                tuples = @[micControls, cameraControls, beautifyControls, forbidChatControls, forbidAudioControls, pptRemarkControls, backgroundAudioControls];
                break;
            }
            default: {
                tuples = @[micControls, cameraControls, beautifyControls, forbidChatControls, pptRemarkControls, backgroundAudioControls];
                break;
            }
        }
    }
        ...
        if(self.room.featureConfig.isWebRTC) {
            ...
            [allSwitchs addObject:self.mirrorModeSwitch];
            tuples = @[micControls, cameraControls, beautifyControls, mirrorModelControls, forbidChatControls, pptRemarkControls, backgroundAudioControls];
        }
        else {
            tuples = @[micControls, cameraControls, beautifyControls, forbidChatControls, pptRemarkControls, backgroundAudioControls];
        }
        ...
        tuples = @[micControls, cameraControls, beautifyControls, backgroundAudioControls];
    }
    
    self.allSwitchs = [allSwitchs copy];
    ...
    NSMutableArray *videoDefinitionButtons = [NSMutableArray new];
    [videoDefinitionButtons bjl_addObject:self.videoDefinitionLowButton];
    [videoDefinitionButtons bjl_addObject:self.videoDefinitionHighButton];
    if (self.room.featureConfig.support720p) {
        [videoDefinitionButtons bjl_addObject:self.videoDefinition720pButton];
    }
    self.videoDefinitionButtons = [videoDefinitionButtons copy];
    ...
    self.pptAnimationLabel = [self makeLabelWithText:@"PPT类型："];
    self.pptAnimationButton = [self makeSelectButtonWithTitle:@"动态"];
    self.pptDisableAnimationButton = [self makeSelectButtonWithTitle:@"静态"];
    ...
    [self makeConstraintsForSubviewsAfterSeparatorLineWithTuples:tuples];
}
...
- (BJLTupleType(UIView *, UISwitch *))makeSwitchWithLabel:(NSString *)text {
    ...
    swiitch.onTintColor = [UIColor bjl_colorWithHex:0X1795FF];
...
- (UIButton *)makeSelectButtonWithTitle:(NSString *)title {
    ...
    UIImage *selectedImage = [UIImage bjl_imageWithColor:[UIColor bjl_colorWithHex:0X1795FF]];
...
- (UIButton *)makeVideoDefinitionButtonWithTitle:(NSString *)title definition:(BJLVideoDefinition)definition {
    ...
    [button bjl_addHandler:^(__kindof UIControl * _Nullable sender) {
        bjl_strongify(self);
        [self.videoDefinitionButtons enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj bjl_disableForSeconds:BJLScRobotDelayM];
        }];
        if (!sender.selected) {
...
- (void)makeConstraintsForSubviewsAfterSeparatorLineWithTuples:(NSArray<BJLTuple *> *)tuples {
    BJLTuple *firstTuple = [tuples bjl_objectAtIndex:0];
    BJLTuple *secondTuple = [tuples bjl_objectAtIndex:1];
    BJLTuple *thirdTuple = [tuples bjl_objectAtIndex:2];
    
    // 课件显示
    BJLTupleUnpack(firstTuple) = ^(UIView *label, UISwitch *swiitch) {
        [self.contentModeLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.centerX.equalTo(swiitch);
            make.centerY.equalTo(self.contentModeFitButton);
        }];
    };
    BJLTupleUnpack(secondTuple) = ^(UIView *label, UISwitch *swiitch) {
    ...
    BJLTupleUnpack(thirdTuple) = ^(UIView *label, UISwitch *swiitch) {
    ...
    for (NSInteger i = 0 ; i < [self.videoDefinitionButtons count]; i++) {
        UIButton *button = [self.videoDefinitionButtons objectAtIndex:i];
        if (i % 2 == 0 ) {
            [button bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
                make.centerX.equalTo(self.contentModeFitButton);
                make.top.equalTo(self.contentModeFitButton.bjl_bottom).with.offset(BJLScViewSpaceL + (BJLScButtonSizeS + BJLScViewSpaceL) * (i/2));
            }];
        }
        else {
            [button bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
                make.centerX.equalTo(self.contentModeFillButton);
                make.top.equalTo(self.contentModeFillButton.bjl_bottom).with.offset(BJLScViewSpaceL + (BJLScButtonSizeS + BJLScViewSpaceL) * (i/2));
            }];
        }
    }
    ...
    [self.cameraPositionFrontButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.centerX.equalTo(self.contentModeFitButton);
        make.top.equalTo(self.contentModeFitButton.bjl_bottom).with.offset(BJLScViewSpaceL + (1 + ([self.videoDefinitionButtons count]-1) / 2) * (BJLScButtonSizeS + BJLScViewSpaceL));
    }];
    ...
    // PPT 类型
    [self.pptAnimationLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.contentModeLabel);
        make.centerY.equalTo(self.pptAnimationButton);
    }];
    
    [self.pptAnimationButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.centerX.equalTo(self.contentModeFitButton);
        make.top.equalTo(self.cameraPositionFrontButton.bjl_bottom).with.offset(BJLScViewSpaceL);
    }];
    
    [self.pptDisableAnimationButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.right.equalTo(self.contentModeFillButton);
        make.centerY.equalTo(self.pptAnimationButton);
    }];
    ...
    [self.upLinkTypeTCPButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.centerX.equalTo(self.contentModeFitButton);
        make.top.equalTo(self.pptAnimationButton.bjl_bottom).with.offset(BJLScViewSpaceL);
    }];
...
- (void)makeObservingAndActions {
    ...
    [self bjl_kvo:BJLMakeProperty(self.room.roomVM, liveStarted) observer:^BJLControlObserving(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
        ...
        self.micSwitch.enabled = mediaEnabled;
        self.cameraSwitch.enabled = mediaEnabled;
        
        // 其他功能switch开关
        [self.allSwitchs enumerateObjectsUsingBlock:^(UISwitch * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            bjl_strongify(self);
            obj.enabled = self.room.roomVM.liveStarted;
        }];
        
        // 本地采集分辨率
        self.videoDefinitionLowButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinitionLowButton] && self.room.roomVM.liveStarted;
        self.videoDefinitionHighButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinitionHighButton] && self.room.roomVM.liveStarted;
        self.videoDefinition720pButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinition720pButton] && self.room.roomVM.liveStarted;
        self.videoDefinition1080pButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinition1080pButton] && self.room.roomVM.liveStarted;
        
        // 上下行链路
        self.upLinkTypeTCPButton.enabled = !self.room.featureConfig.isWebRTC && self.room.roomVM.liveStarted;
        self.upLinkTypeUDPButton.enabled = !self.room.featureConfig.isWebRTC && self.room.roomVM.liveStarted;
        self.downLinkTypeTCPButton.enabled = !self.room.featureConfig.isWebRTC && self.room.roomVM.liveStarted;
        self.downLinkTypeUDPButton.enabled = !self.room.featureConfig.isWebRTC && self.room.roomVM.liveStarted;
        return YES;
    }];
    ...
    [self bjl_kvo:BJLMakeProperty(self.room.recordingVM, isOnEncoderMirrorMode)
         observer:^BJLControlObserving(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
        bjl_strongify(self);
        self.mirrorModeSwitch.on = self.room.recordingVM.isOnEncoderMirrorMode;
        return YES;
    }];
    [self.mirrorModeSwitch bjl_addHandler:^(UISwitch * _Nullable sender, UIControlEvents event) {
        bjl_strongify(self);
        [sender bjl_disableForSeconds:BJLScRobotDelayS];
        [self.room.recordingVM sendEncoderMirrorMode:sender.on];
    } forControlEvents:UIControlEventValueChanged];
    ...
    [self.contentModeFitButton bjl_addHandler:^(__kindof UIControl * _Nullable sender) {
        ...
        if (!self.contentModeFitButton.selected) {
            [self.room.slideshowViewController updateContentMode:BJLContentMode_scaleAspectFit];
        }
    }];
    [self.contentModeFillButton bjl_addHandler:^(__kindof UIControl * _Nullable sender) {
        ...
        if (!self.contentModeFillButton.selected) {
            ...
            [self.room.slideshowViewController updateContentMode:BJLContentMode_scaleAspectFill];
        }
    }];
    
    // PPT切换动态/静态
    [self.pptAnimationButton bjl_addHandler:^(UIButton * _Nonnull button) {
        bjl_strongify(self);
        [button bjl_disableForSeconds:BJLScRobotDelayS];
        self.room.disablePPTAnimation = NO;
    }];
    
    [self.pptDisableAnimationButton bjl_addHandler:^(UIButton * _Nonnull button) {
        bjl_strongify(self);
        [button bjl_disableForSeconds:BJLScRobotDelayS];
        self.room.disablePPTAnimation = YES;
    }];
    ...
    [self bjl_kvoMerge:@[BJLMakeProperty(self.room.documentVM, allDocuments),
        ...
        self.pptAnimationButton.enabled = hasAnimatedPPT && !self.room.featureConfig.disablePPTAnimation;
        self.pptAnimationButton.selected = !self.room.disablePPTAnimation && hasDocument;
        
        self.pptDisableAnimationButton.enabled = hasDocument;
        self.pptDisableAnimationButton.selected = self.room.disablePPTAnimation && hasDocument;
    }];
    ...
    [self bjl_kvo:BJLMakeProperty(self.room.recordingVM, recordingVideo)
         observer:^BJLControlObserving(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
        bjl_strongify(self);
        self.cameraPositionRearButton.enabled = self.room.recordingVM.recordingVideo;
        self.cameraPositionFrontButton.enabled = self.room.recordingVM.recordingVideo;
        return YES;
    }];
    ...
    [self bjl_kvo:BJLMakeProperty(self, videoDefinitionButtons) observer:^BJLControlObserving(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
        bjl_strongify(self);
        self.videoDefinitionLowButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinitionLowButton] && self.room.roomVM.liveStarted;
        self.videoDefinitionHighButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinitionHighButton] && self.room.roomVM.liveStarted;
        self.videoDefinition720pButton.enabled = [self.videoDefinitionButtons containsObject:self.videoDefinition720pButton] && self.room.roomVM.liveStarted;
