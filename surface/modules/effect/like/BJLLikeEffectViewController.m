...
- (instancetype)initWithName:(NSString *)name {
    if (self = [super init]) {
        ...
        for (NSInteger i = 1; i <= 20; i ++) {
            UIImage *image = [UIImage bjlsc_imageNamed:[NSString stringWithFormat:@"bjl_like_star%ld", (long)i]];
...
- (void)makeSubviewsAndConstraints {
    ...
    self.awardBackgroundImageView = ({
        ...
        imageView.image = [UIImage bjlsc_imageNamed:@"bjl_like_shine"];
        imageView;
    });
    ...
    [self bjl_kvo:BJLMakeProperty(self.awardImageView, bounds)
         observer:^BOOL(id  _Nullable now, id  _Nullable old, BJLPropertyChange * _Nullable change) {
             ...
             if (self.awardImageView.bounds.size.height) {
                 CGFloat bottomOffset = self.awardImageView.bounds.size.height * 0.15;
                 ...
                 [self.nameLabel bjl_remakeConstraints:^(BJLConstraintMaker *make) {
                     ...
                     make.width.lessThanOrEqualTo(self.awardImageView.bjl_width).multipliedBy(0.5);
...
- (void)updateAwardImageView {
    if (self.isForInteractiveClass && self.interactiveType == BJLInteractiveTypeGroupAward) {
        self.awardImageView.image = [UIImage bjlsc_imageNamed:@"bjl_groupAward"];
    }
    else if (self.isForInteractiveClass && self.interactiveType == BJLInteractiveTypeClassAward) {
        self.awardImageView.image = [UIImage bjlsc_imageNamed:@"bjl_classAward"];
    }
    else {
        if (self.imageUrlString.length > 0) {
            [self.awardImageView bjl_setImageWithURL:[NSURL URLWithString:self.imageUrlString] placeholder:[UIImage bjlsc_imageNamed:@"bjl_like_award"] completion:nil];
        }
        else {
            self.awardImageView.image = [UIImage bjlsc_imageNamed:@"bjl_like_award"];
        }
    }
}
