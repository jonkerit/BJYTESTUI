//
//  BJLScPPTQuickSlideViewController.m
//  BJLiveUI
//
...
@interface BJLScPPTQuickSlideViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation BJLScPPTQuickSlideViewController
...
- (void)viewDidLoad {
    ...
    self.view.layer.borderColor = [UIColor bjlsc_grayBorderColor].CGColor;
...
- (void)makeObserving {
    ...
    [self bjl_kvoMerge:@[BJLMakeProperty(self.room.documentVM, currentSlidePage),
              ...
              observer:^(id  _Nullable value, id  _Nullable oldValue, BJLPropertyChange * _Nullable change) {
                  ...
                  if (pageIndex + 1 <= whiteboardCount) {
                      ...
                      self.localWhiteboardIndex = pageIndex;
                      self.loaclPageIsWhiteboard = YES;
                  }
                  else {
                      ...
                      self.localPPTPreviewIndex = pageIndex - whiteboardCount;
                  }
                  [self reloadCollectionView];
                  [self scrollToLocalSlidePage];
              }];
...
- (void)updatePPTContainerViewConstraints {
    if (self.pullToMiniSize) {
        self.pullImageView.image = [UIImage bjlsc_imageNamed:@"bjl_sc_pptpull_min"];
        ...
        self.pullImageView.image = [UIImage bjlsc_imageNamed:@"bjl_sc_pptpull_max"];
        [self.pptContainerView bjl_updateConstraints:^(BJLConstraintMaker * _Nonnull make) {
...
- (void)updatePPTPreviewCollectionViewHidden:(BOOL)hidden {
    if (hidden) {
        ...
        [self.whiteboardCollectionView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
            ...
            make.height.greaterThanOrEqualTo(@(60.0 + BJLScViewSpaceM * 2)).priorityHigh();
        }];
    }
    else {
...
- (void)setUpCollectionView {
    self.addWhiteboardButton = ({
        ...
        [button setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_ppt_add"] forState:UIControlStateNormal];
    ...
    self.whiteboardCollectionView = ({
        ...
        [collectionView registerClass:[BJLScPPTQuickSlideCell class] forCellWithReuseIdentifier:whiteboardCellReuseIdentifier];
        collectionView;
    });
    ...
    self.pullImageView = ({
        ...
        imageView.image = [UIImage bjlsc_imageNamed:@"bjl_sc_pptpull_max"];
        imageView;
    });
    ...
    self.pptPreviewCollectionView = ({
        ...
        collectionView.dataSource = self;
        collectionView.delegate = self;
        [collectionView registerClass:[BJLScPPTQuickSlideCell class] forCellWithReuseIdentifier:pptPreviewCellReuseIdentifier];
        collectionView;
    });
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger whiteboardCount = [self.room.documentVM documentWithID:BJLBlackboardID].pageInfo.pageCount;
    if (collectionView == self.pptPreviewCollectionView) {
        BJLScPPTQuickSlideCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:pptPreviewCellReuseIdentifier forIndexPath:indexPath];
        ...
        BJLScPPTQuickSlideCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:whiteboardCellReuseIdentifier forIndexPath:indexPath];
        [cell setSingleTapCallback:^{
            bjl_strongify(self);
...
- (void)selectWhiteboardCollectionViewAtIndex:(NSInteger)index {
    BJLError *error = [self.room.slideshowViewController updatePageIndex:index];
    if (error) {
        [self showProgressHUDWithText:error.localizedFailureReason ?: error.localizedDescription];
    }
    else {
...
- (void)selectPPTPreviewCollectionViewAtIndex:(NSInteger)index {
    NSInteger whiteboardCount = [self.room.documentVM documentWithID:BJLBlackboardID].pageInfo.pageCount;
    BJLError *error = [self.room.slideshowViewController updatePageIndex:whiteboardCount + index];
    if (error) {
        [self showProgressHUDWithText:error.localizedFailureReason ?: error.localizedDescription];
    }
    else {
