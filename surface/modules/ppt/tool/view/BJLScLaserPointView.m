...
@interface BJLScLaserPointView ()

@property (nonatomic, readonly, weak) BJLRoom *room;
@property (nonatomic) UIImageView *laserPointView;
@property (nonatomic) NSString *laserPointImageName;
...
@implementation BJLScLaserPointView
...
- (void)setupSubviews {
    self.laserPointView = ({
        UIImage *image = [UIImage bjlsc_imageNamed:@"bjl_sc_laserpoint"];
        self.laserPointImageName = @"bjl_sc_laserpoint";
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        imageView.frame = CGRectMake(0.0, 0.0, 16, 16);
...
- (void)setupObservers {
    ...
    [self bjl_observe:BJLMakeMethod(self.room.drawingVM, didLaserPointMoveToLocation:documentID:pageIndex:)
         observer:^BOOL(CGPoint location, NSString *documentID, NSUInteger pageIndex) {
             bjl_strongify(self);
             [self updateLaserPointViewWithImageName:@"bjl_sc_laserpoint"];
    ...
    [self bjl_observe:BJLMakeMethod(self.room.drawingVM, didMousePointMoveToLocation:documentID:pageIndex:)
         observer:^BOOL(CGPoint location, NSString *documentID, NSUInteger pageIndex) {
             bjl_strongify(self);
             [self updateLaserPointViewWithImageName:@"bjl_sc_mousepoint"];
             self.laserPoint = location;
             CGSize showSize = self.showSize;
             if (CGSizeEqualToSize(showSize, CGSizeZero)) {
                 showSize = self.bounds.size;
...
- (void)updateLaserPointViewWithImageName:(NSString *)imageName {
    if (!imageName.length || [self.laserPointImageName isEqualToString:imageName]) {
        return;
    }
    self.laserPointImageName = imageName;
    self.laserPointView.image = [UIImage bjlsc_imageNamed:imageName] ?: [UIImage bjlsc_imageNamed:@"bjl_sc_laserpoint"];
}
