//
//  BJLScNoticeEditViewController.m
//  BJLiveUI
//
...
@interface BJLScNoticeEditViewController ()<UITextViewDelegate, UITextFieldDelegate>
...
@property (nonatomic) UIButton *doneButton, *giveupEditButton;

@end

@implementation BJLScNoticeEditViewController
...
- (void)makeSubviews {
    ...
    self.emptyGroupNoticeView = ({
        ...
        UIImageView *imageView = [UIImageView new];
        [imageView setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_usergroup_open"]];
        [view addSubview:imageView];
        [imageView bjl_makeConstraints:^(BJLConstraintMaker * _Nonnull make) {
    ...
    if (MIN(screenSize.width, screenSize.height) > 320.0) {
        self.linkTextField = ({
            ...
            textField.textColor = [UIColor bjlsc_darkGrayTextColor];
...
- (void)makeConstraints {
    ...
    [self.scrollView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.contentView.bjl_bottom).with.offset(BJLScViewSpaceL * 2);
    }];
}
...
- (void)udateEditView {
    ...
    if (self.room.loginUser.groupID == 0) {
        [self.editImageView setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_notice_inClass"]];
        ...
        [self.editImageView setImage:[UIImage bjlsc_imageNamed:@"bjl_sc_notice_inGroup"]];
    ...
    [self.editNoticeTextView bjl_remakeConstraints:^(BJLConstraintMaker * _Nonnull make) {
        make.top.equalTo(self.editView).offset(40 + BJLScViewSpaceL);
        make.left.right.equalTo(@[self.view.bjl_safeAreaLayoutGuide ?: self.view, self.scrollView]).with.inset(BJLScViewSpaceL);
    ...
    [self.scrollView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        make.edges.equalTo(self.view);
        make.bottom.equalTo(self.editView.bjl_bottom).with.offset(BJLScViewSpaceL * 2);
    }];
...
- (void)done {
    ...
    if (self.doneButton.selected) {
        ...
        if (self.room.loginUser.isAssistant && self.room.loginUser.noGroup && ![self.room.roomVM getAssistantaAuthorityWithNotice]) {
            [self showProgressHUDWithText:@"公告编辑权限已被禁用"];
            return ;
        }
