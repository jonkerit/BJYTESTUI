...
- (void)makeSubviews {
    self.headerContainerView = ({
        ...
        [line bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.left.equalTo(view).with.offset(BJLScViewSpaceL);
...
- (void)makeConstraints {
    ...
    [self.titleLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.headerContainerView.bjl_safeAreaLayoutGuide ?: self.headerContainerView).with.offset(BJLScViewSpaceL);
    ...
    [self.buttonsStackView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        ...
        make.left.greaterThanOrEqualTo(self.titleLabel.bjl_right).with.offset(BJLScViewSpaceL);
...
- (void)updateHeaderViewConstraints {
    ...
    CGFloat headerHeight = shown ? BJLScControlSize : 0.0;
...
- (void)updateRightButtons:(nullable NSArray<UIButton *> *)rightButtons {
    ...
    for (UIButton *button in self.rightButtons) {
        ...
        [button bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.right.equalTo(last.bjl_left ?: self.buttonsStackView).with.offset(- (last ? BJLScViewSpaceM : BJLScViewSpaceL));
