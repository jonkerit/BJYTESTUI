...
@property (nonatomic, nullable) void (^remakeConstraintsBlock)(BJLConstraintMaker *make, UIView *superView, BOOL isHorizontalUI, BOOL isHorizontalSize);
...
- (void)viewDidLoad {
    ...
    UITapGestureRecognizer *tap = [UITapGestureRecognizer bjl_gestureWithHandler:^(__kindof UIGestureRecognizer * _Nullable gesture) {
        bjl_strongify(self);
        if (!self.tapBackgroundToHide) return;
        CGPoint location = [gesture locationInView:gesture.view];
        UIView *target = [gesture.view hitTest:location withEvent:nil];
        if (target == self.view) {
            [self hide];
        }
    }];
...
- (void)updateConstraintsForHorizontal:(BOOL)isHorizontal {
    [self.containerController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
...
- (void)showWithContentViewController:(UIViewController *)contentViewController
               remakeConstraintsBlock:(void (^)(BJLConstraintMaker *make, UIView *superView, BOOL isHorizontalUI, BOOL isHorizontalSize))remakeConstraintsBlock {
    if (!self.view.hidden) {
        [self hide];
    }
...
- (void)showWithContentViewController:(UIViewController *)contentViewController
    ...
    if (@available(iOS 13.0, *)) {
        contentViewController.overrideUserInterfaceStyle = UIUserInterfaceStyleLight;
    }
