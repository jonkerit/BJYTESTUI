...
@property (nonatomic, readwrite) PHPickerResult *pickerResult API_AVAILABLE(ios(14));
...
+ (nullable instancetype)imageFileWithPickerResult:(PHPickerResult *)pickerResult thumbnail:(nullable UIImage*)thumbnail filePath:(NSString *)filePath error:(NSError *__autoreleasing  _Nullable *)error {
    ICLImageFile *imageFile = [self new];
    if (@available(iOS 14, *)) {
        imageFile.pickerResult = pickerResult;
    }
    imageFile.thumbnail = thumbnail;
    imageFile.filePath = filePath;
...
- (void)icl_alertError {
    UIAlertController *alert = [UIAlertController
                                bjl_lightAlertControllerWithTitle:@"无法下载照片"
...
- (UIAlertController *)icl_loadingAlertWithProgressView:(UIView *)progressView
    ...
    UIAlertController *alert = [UIAlertController
                                bjl_lightAlertControllerWithTitle:@"正在从 iCloud 加载..."
                                message:@"\n\n\n\n\n"
                                preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction
                      actionWithTitle:@"取消"
